...
def linear_quantize(input, scale, zero_point):
    ...
    """
    # Get the number of bits for the mantissa and exponent of minifloat
    mantissa_bits = 10
    exponent_bits = 5

    # Calculate the maximum representable value for the given bit configuration
    max_value = (2**(mantissa_bits + 1) - 1) * (2**(2**exponent_bits))

    # TODO: Quantize the input tensor using minifloat quantization (start)
    output = torch.zeros_like(input)
    clamped_input = torch.clamp(input / scale, 2**(-mantissa_bits - 1), max_value / scale)
    mantissa, exponent = clamped_input.frexp()
    rounded_mantissa = torch.round(mantissa * (2**mantissa_bits))
    quantized = torch.ldexp(rounded_mantissa, exponent).round() - zero_point
    output = torch.clamp(quantized, 0, 2**mantissa_bits - 1)
    # TODO: Quantize the input tensor using minifloat quantization (end)
...
def conv2d_linear_quantized(
    ...
    # TODO: fill-in (start)
    y = module(quantize_activations(x, a_qconfig, fake_quantize=True))
    # TODO: fill-in (end)
